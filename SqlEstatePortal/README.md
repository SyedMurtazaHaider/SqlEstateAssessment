...
> **Deploying to a server?** Follow the full guide: **[DEPLOYMENT.md](DEPLOYMENT.md)** (this folder)  
...
```text
SqlEstateAssessment\
  SqlEstateAssessment.sln
  SqlEstatePortal\              # ASP.NET Core MVC app + collector
    README.md                   # This file
...
3. Copy the publish output to `C:\Apps\SqlEstateAssessment\SqlEstatePortal\` (includes `Assess-SqlEstate.ps1`). Copy `servers.example.txt` to `servers.txt` and create `reports\`.  
